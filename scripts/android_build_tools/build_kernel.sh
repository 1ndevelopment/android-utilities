...
git clone --recursive --branch $KERNEL_BRANCH $KERNEL_URL $WORKSPACE --depth=1

cd $WORKSPACE

# Grab toolchains

git clone --recursive --branch $GAS_BRANCH $GAS_URL gas --depth=1
git clone --recursive --branch $CLANG_BRANCH $CLANG_URL clang --depth=1

# Add toolchains to PATH

export PATH=$WORKSPACE/clang/bin:$WORKSPACE/clang:$WORKSPACE/gas/bin:$WORKSPACE/gas:$PATH
export LD_LIBRARY_PATH=$WORKSPACE/clang/lib64:$LD_LIBRARY_PATH

# Build from _defconfig

make -j4 O=$WORKSPACE/out $BUILD_FLAGS k65v1_64_bsp_defconfig

# Build kernel

make -j4 O=$WORKSPACE/out $BUILD_FLAGS CONFIG_DEBUG_SECTION_MISMATCH=y
